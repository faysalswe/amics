    ...
    public class PartMasterService:IPartMasterService
    {
        ...
        public List<LstLocation> LocationLookup(string searchLocation,string warehouseId)
        {
            ...
            var result = _amicsDbContext.lstLocations
                .FromSqlInterpolated($"select id,location from list_locations where warehousesid={whouseId} and location like  {"%" + searchLocation + "%"} and isnull(flag_delete,0)=0 order by location")
                .ToList();
